...
const createNominateOrganizer =
  (contractInteractor: ContractInteractor, config: Config) =>
  async (input: NominateOrganizerInput): Promise<NominateOrganizerResult> => {
...
export const useNominateOrganizer = (
  ...
  const nominateOrganizer = useMemo(
    () => createNominateOrganizer(contractInteractor, config),
    [contractInteractor, config]
  )

  return useMutation({
    mutationFn: () => nominateOrganizer(input),
    onError: error => {
