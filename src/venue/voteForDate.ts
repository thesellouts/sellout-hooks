...
const createVoteForDate =
  (contractInteractor: ContractInteractor, config: Config) =>
  async (input: VoteForDate): Promise<VoteForDateResult> => {
...
export const useVoteForDate = (
  ...
  const voteForDate = useMemo(
    () => createVoteForDate(contractInteractor, config),
    [contractInteractor, config]
  )

  return useMutation({
    mutationFn: () => voteForDate(input),
    onError: error => {
